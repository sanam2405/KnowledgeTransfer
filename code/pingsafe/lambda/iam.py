...
AWS_IAM_ACCESS_KEYS_LAST_USED_THRESHOLD = (int)(
    os.environ["AWS_IAM_ACCESS_KEYS_LAST_USED_THRESHOLD"]
)  # in days
...
def delete_access_key(iam_client, username, accesskeyid):
    iam_client.update_access_key(
        UserName=username, AccessKeyId=accesskeyid, Status="Inactive"
    )
            ...
            for acc in response["AccessKeyMetadata"]:
                ...
                if "AccessKeyLastUsed" in resp.keys():
                    if "LastUsedDate" in resp["AccessKeyLastUsed"].keys():
                        dt_access_key_last_used = (
                            resp["AccessKeyLastUsed"]["LastUsedDate"]
                        ).replace(tzinfo=None)
                    dt_now = datetime.now()

                    if (
                        dt_now - dt_access_key_last_used
                    ).days > AWS_IAM_ACCESS_KEYS_LAST_USED_THRESHOLD:
                        delete_access_key(iam_client, iam_user_name, access_key_id)
                        disabled_access_keys.append(
                            "{}: {}".format(iam_user_name, access_key_id)
                        )

                # Access Key created but never used
                elif (
                    "ServiceName" in resp["AccessKeyLastUsed"].keys()
                    and resp["AccessKeyLastUsed"]["ServiceName"] == "N/A"
                ):
                    ...
                    if "AccessKeyMetadata" in resp2.keys():
                        tmp = [
                            j["CreateDate"]
                            for j in resp2["AccessKeyMetadata"]
                            if j["AccessKeyId"] == access_key_id
                        ]
                        if len(tmp) > 0:
                            ...
                            if (
                                ...
                                delete_access_key(
                                    iam_client, iam_user_name, access_key_id
                                )
                                disabled_access_keys.append(
                                    "{}: {}".format(iam_user_name, access_key_id)
                                )
...
def remediate_aws_iam_userpassword_last_used(pingsafe_event, role_to_assume):
    ...
    if len(deleted_profiles) > 0:
        send_slack(
            "Account: {}\n\n".format(accountname)
            + "Deleted Login Profiles for users (password not used in 90 days):\n"
...
def remediate_aws_iam_accesskeys_not_rotated(pingsafe_event, role_to_assume):
    ...
    for j in pingsafe_event:
        ...
        print(
            "{}/{}, Please rotate access keys".format(
                j["accountTitle"], j["resourceId"]
            )
        )
...
def remediate_aws_iam_users_mfa_disabled(pingsafe_event, role_to_assume):
    ...
    for j in pingsafe_event:
        ...
        send_slack(
            "AccountID: {}\n\n".format(accountid)
            + "Please Enable MFA: {} in {}".format(j["resourceId"], j["accountTitle"])
        )
...
def remediate_aws_iam_empty_groups(pingsafe_event, role_to_assume):
    ...
    for j in pingsafe_event:
        ...
        try:
            ...
            ret_attached_policies = iam_client.list_attached_group_policies(
            ...
            if "AttachedPolicies" in ret_attached_policies.keys():
                for k in ret_attached_policies["AttachedPolicies"]:
                    iam_client.detach_group_policy(
                        PolicyArn=k["PolicyArn"], GroupName=group_name
                    )
    ...
    if "InstanceProfile" in ret.keys() and "Roles" in ret["InstanceProfile"].keys():
        for k in ret["InstanceProfile"]["Roles"]:
            ...
            iam_client.remove_role_from_instance_profile(
                InstanceProfileName=ip_name, RoleName=role_name_2
            )
...
def get_instance_profiles(ec2_client, token="-1"):
    ...
    for j in ret_tmp["Reservations"]:
        ret += [
            k["IamInstanceProfile"]["Arn"]
            for k in j["Instances"]
            if (
                "IamInstanceProfile" in k.keys()
                and "Arn" in k["IamInstanceProfile"].keys()
            )
        ]
...
def remediate_unused_iam_roles(pingsafe_event, role_to_assume):
    ...
    for j in pingsafe_event:
        ...
        if "Role" in role_data.keys() and "RoleLastUsed" in role_data["Role"].keys():
            ...
            if (
                "Path" in role_data["Role"].keys()
                and "service-role/" in role_data["Role"]["Path"]
            ):
                continue
            ...
            if "LastUsedDate" in role_data["Role"]["RoleLastUsed"].keys():
                dt = (role_data["Role"]["RoleLastUsed"]["LastUsedDate"]).replace(
                    tzinfo=None
                )
            ...
            if (dt_now - dt).days >= UNUSED_IAM_ROLES_THRESHOLD:
                ...
                try:
                    ...
                    slack_msg.append(
                        "IAM Role: {}, Last Used: {} days ago".format(
                            role_name, (dt_now - dt).days
                        )
                    )
                    ...
                    role_instance_profile_data = iam_client.list_instance_profiles_for_role(
                        RoleName=role_name
                    )
                    role_active = False
                    if "InstanceProfiles" in role_instance_profile_data.keys():
                        for prof in role_instance_profile_data["InstanceProfiles"]:
                            if (
                                prof["InstanceProfileName"]
                                not in instance_profiles_active_list
                            ):
                                delete_instance_profile(
                                    iam_client, prof["InstanceProfileName"]
                                )
                                slack_msg.append(
